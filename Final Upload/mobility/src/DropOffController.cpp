...
void DropOffController::calculateDecision() {
    ...
    if(reachedCollectionPoint)
    {
        ...
        //we can now use it for our timing of 2 seconds

        if (timerTimeElapsed >= 6)//original(4)
        {
            result.reset = true; //tell mobility to reset to search parameters
        }
        else if (timerTimeElapsed >= 3.0)//original(1.0)
        {
            //open fingers
            float angle;
            angle = M_PI_2;
            ...
            result.cmdVel = -1;//original(-0.3)
            result.angleError = 45.0;//original(0.0)
        }
        return;
    }
