...
PickUpResult PickUpController::pickUpSelectedTarget(bool blockBlock) {
  ...
  result.giveUp = false;*/
    ...
    {
        lockTarget = true;
        result.cmdVel = 0.25;//Original(0.18)
        result.angleError = 0.0;
        timeOut = true;
    }
    else if (Td > 2.4) //raise the wrist
    {
        result.cmdVel = -0.25;
        result.angleError = 0.0;
        result.wristAngle = 0.0;
    }
    else if (Td > 1.5) //close the fingers and stop driving//original(1.7)
    {
        result.cmdVel = -0.15;//original(-0.1)
        result.angleError = 0.0;
    ...
    if (Td > 3.8 && timeOut) //if enough time has passed enter a recovery state to re-attempt a pickup
    {
        ...
        {
            lockTarget = false;
            result.cmdVel = -.30;//original(-0.15)
            result.angleError = 0.0;
            //set gripper
